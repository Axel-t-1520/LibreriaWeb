...
export const descargarPDFFactura = async (req, res) => {
  try {
    ...
    if (error || !factura) {
      return res
        .status(404)
        .json({ message: "Factura no encontrada", error: error?.message });
    }

    if (
      !factura.Cliente ||
      !factura.Vendedor ||
      !factura.Detalle_Factura?.length
    ) {
      return res
        .status(404)
        .json({ message: "Datos de la factura incompletos" });
    }
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=factura-${factura.codigo || facturaId}.pdf`,
    );
    ...
    pdf
      ...
      .text("Teléfono: 64058719", { align: "center" })
      .moveDown(1);
    ...
    const printField = (label, value, y) => {
      ...
      pdf
        .font("Helvetica")
        .text(value, margin + 50, y, { width: contentWidth - 50 });
    };
    ...
    const fechaStr =
      fechaObj.toLocaleDateString("es-BO", opciones) +
      " " +
      fechaObj.toLocaleTimeString("es-BO", opcionesHora);
    ...
    const col3 = contentWidth * 0.2;
    const col4 = contentWidth * 0.2;
    ...
    pdf.text("P. Unit.", margin + col1 + col2, rowY, {
      width: col3,
      align: "right",
    });
    pdf.text("Subtotal", margin + col1 + col2 + col3, rowY, {
      width: col4 - 3,
      align: "right",
    });
    ...
    productosValidos.forEach((producto, index) => {
      ...
      pdf.text(producto.nombre, margin + 3, yPos, {
        width: col1 - 6,
        ellipsis: true,
      });
      pdf.text(producto.cantidad.toString(), margin + col1, yPos, {
        width: col2,
        align: "center",
      });
    ...
    pdf.text("TOTAL:", margin + col1 + col2, yPos, {
      width: col3,
      align: "right",
    });
    ...
    pdf
      .fontSize(11)
      .text(`Bs. ${totalVenta.toFixed(2)}`, margin + col1 + col2 + col3, yPos, {
        width: col4 - 3,
        align: "right",
      });
    ...
    pdf
      ...
      .text("Gracias por su preferencia", margin, pageHeight - 30, {
        align: "center",
        width: contentWidth,
      });
